...
/**
 * Panel de dessin du graphe MPM.
 * Gère l'affichage et l'interaction avec les entités du graphe,
 * incluant le zoom, le déplacement et les menus contextuels.
 */
                                                        ...
                                                        ActionListener
{
     ...
     *--------------*/
    /**
     * Crée un nouveau panel de graphe avec ses composants.
     * Initialise les menus contextuels et les écouteurs d'événements.
     * @param parent Panel parent MPM
     ...
     *----------------------------------*/
    /**
     * Définit la liste des entités à afficher.
     * @param entites Liste des entités
     */
     ...
     *----------------------------------*/
    /**
     * Met à jour la taille du panel en fonction des entités.
     */
     ...
     * @return Tableau contenant la largeur et la hauteur nécessaires
     */
    ...
    /**
     * Dessine les dates sur une entité si nécessaire.
     * @param g2 Contexte graphique
     * @param entite Entité à annoter
     */
    private void dessinerDatesSurEntite(Graphics2D g2, Entite entite) 
    {
        ...
        if ( afficherDateTot  && entite.getNiveauTache() <= numNiveauxTot  ) 
            this.dessinerDateTot (g2, entite, fm);
        if ( afficherDateTard && entite.getNiveauTache() >= numNiveauxTard ) 
            this.dessinerDateTard(g2, entite, fm);
    }
    ...
    /**
     * Formate le texte de la date selon le format choisi.
    ...
    /**
     * Calcule les coordonnées pour le texte des dates.
    ...
    /**
     * Dessine les connexions entre les entités.
     * @param g2 Contexte graphique
     */
    private void dessinerConnexions(Graphics2D g2) 
    { 
        this.parentPanel.dessinerConnexions(g2); 
    }
     ...
     *----------------------------------*/
    /**
     * Trouve une entité à la position spécifiée.
    ...
    public void mouseWheelMoved(MouseWheelEvent e) 
    {
        if (e.isControlDown() || e.isMetaDown() || e.getPreciseWheelRotation() != 0) 
        {
            ...
            this.scale = Math.max(0.2, Math.min(this.scale, 5.0));
            revalidate();
            repaint();
        }
    }
